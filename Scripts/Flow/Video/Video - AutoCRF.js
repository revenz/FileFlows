...
function Script() {
    ...
    if (Variables.HardwareAcceleration) {
        HardwareAcceleration = Variables.HardwareAcceleration;
    }
    HardwareAcceleration = HardwareAcceleration.toLowerCase();
    ...
    if (videoBitRate > targetBitRate) {
        ...
        Logger.WLog(
            `Bitrate is ${bytesToMB(
                videoBitRate
            )} MBps, higher than ${MaxBitRate} MBps`
        );
    ...
    if (!forceEncode && videoCodec == TargetCodec) {
        Logger.ILog(
            `Bitrate (${bytesToMB(
                videoBitRate
            )} MBps) and codec (${videoCodec}) are acceptable`
        );
        if (UseTags) {
            Flow.AddTags(["Copy"]);
        }
    ...
    let attempt = search(firstTryPercentage, firstTryScore);
    if (attempt.winner) {
        Variables.ManualParameters = `${attempt.command} -global_quality:v ${attempt.winner.crf}`;
        Logger.ILog(
            `First attempt successful with ${attempt.winner.size}% size, ${attempt.winner.score}% VMAF`
        );
    ...
    attempt = search(secondTryPercentage, secondTryScore);
    if (attempt.winner) {
        Variables.ManualParameters = `${attempt.command} -global_quality:v ${attempt.winner.crf}`;
        Logger.ILog(
            `Second attempt successful with ${attempt.winner.size}% size, ${attempt.winner.score}% VMAF`
        );
        Flow.AdditionalInfoRecorder("Score", attempt.winner.score, 1000);
        Flow.AdditionalInfoRecorder("CRF", attempt.winner.crf, 1000);
        if (UseTags) {
...
function search(bitratePercent, targetPercent) {
    let abAv1 = ToolPath("ab-av1", "/opt/autocrf");
    let path = abAv1.replace(/[^\/]+$/, "");

    let targetCodec = TargetCodec;
    if (HardwareAcceleration == "intel") {
        targetCodec = `${TargetCodec}_qsv`;
    }
    if (HardwareAcceleration == "nvidia") {
        targetCodec = `${TargetCodec}_nvenc`;
    }
    ...
    let returnValue = {
        ...
        error: false,
        winner: null,
        command: `${targetCodec} -preset slow`,
        message: "",
    };
    ...
    let targetBitRate = (bitratePercent / 100) * Variables.vi.VideoInfo.Bitrate;

    Logger.ILog(
        `Searching for CRF under ${bytesToMB(
            targetBitRate
        )} MBps @ ${targetPercent}% original quality`
    );

    let fileName = Flow.IsWindows
...
function bytesToHuman(b) {
    ...
    return (u ? b.toFixed(0) + " " : b) + " KMGTPEZY"[u] + "Bps";
}
