...
# Step 1: Check if ffmpeg is already installed by checking the full path
FFMPEG_PATH=$(command -v ffmpeg)

if [ -n "$FFMPEG_PATH" ]; then
    if [ "$FFMPEG_PATH" == "$INSTALL_DIR/ffmpeg" ]; then
        echo "FFmpeg is already installed in $INSTALL_DIR."
    else
        echo "FFmpeg is installed in $FFMPEG_PATH. Skipping installation."
    fi
    exit 0
fi
...
# Step 6: Remove any existing ffmpeg and ffprobe symlinks from /usr/local/bin
echo "Removing existing ffmpeg and ffprobe symlinks from $INSTALL_DIR..."
sudo rm -f "$INSTALL_DIR/ffmpeg"
sudo rm -f "$INSTALL_DIR/ffprobe"

# Step 7: Symlink the new ffmpeg and ffprobe from /opt/ffmpeg-static to /usr/local/bin
