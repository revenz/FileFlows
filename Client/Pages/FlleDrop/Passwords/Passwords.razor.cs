...
public partial class Passwords 
{
    ...
    private async Task Refresh(bool blocker = true)
    {
        ...
        var response = await HttpHelper.Get<FileFlows.Shared.Models.FileDropSettings>("/api/file-drop/passwords");
        if (response.Success)
        {
    ...
    private async Task Save()
    {
        ...
        try
        {
            await HttpHelper.Put<string>("/api/file-drop/passwords", Model);
        }
        finally
        {
            IsSaving = false;
