...
public partial class CustomCss 
{
    ...
    /// The help URL
    /// </summary>
    private const string HelpUrl = "https://fileflows.com/docs/file-drop/config/custom-css";
    ...
    private async Task Refresh(bool blocker = true)
    {
        ...
        var response = await HttpHelper.Get<string>("/api/file-drop/custom-css");
        if (response.Success)
        {
    ...
    private async Task Save()
    {
        ...
        try
        {
            await HttpHelper.Put<string>("/api/file-drop/custom-css", Model);
        }
        finally
        {
            IsSaving = false;
