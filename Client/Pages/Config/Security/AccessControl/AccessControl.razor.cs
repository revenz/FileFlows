...
public partial class AccessControl: ListPage<Guid, AccessControlEntry>
{
    ...
    async Task<bool> Save(ExpandoObject model)
    {
        ...
        try
        {
            ...
            {
                feService.Notifications.ShowError(Translater.Instant("Pages.AccessControl.Messages.InvalidIPAddress", new { address = start }));
                return false;
            }
            ...
            if (string.IsNullOrWhiteSpace(end) == false)
            {
                ...
                {
                    feService.Notifications.ShowError(Translater.Instant("Pages.AccessControl.Messages.InvalidIPAddress", new { address = end }));
                    return false;
                }
                
                if(ipStart.AddressFamily != ipEnd.AddressFamily)
                {
                    feService.Notifications.ShowError(Translater.Instant("Pages.AccessControl.Messages.FamilyMismatch"));
                    return false;
                }

                if (IPHelper.IsGreaterThan(ipStart, ipEnd) == false)
                {
                    feService.Notifications.ShowError(Translater.Instant("Pages.AccessControl.Messages.EndMustBeGreaterThan"));
                    return false;
                }
            }
