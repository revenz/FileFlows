...
public partial class InputListString : Input<List<string>>
{
    [Parameter]
    ...
    private void OnKeyDown(KeyboardEventArgs e)
    {
        if (e.ShiftKey == false && e.AltKey == false && e.CtrlKey == false)
        {
            ...
            else if (e.Code == "Backspace" && PreviousInputText == string.Empty)
            {
                ...
                {
                    this.Value = this.Value.Take(this.Value.Count - 1).ToList();
                }
            }
        }
    ...
    {
        this.Value = this.Value.Except([str]).ToList();
    }
    ...
    bool Add(string str)
    {
        ...
        this.Value = this.Value.Union([str]).ToList();
        return true;
    }
